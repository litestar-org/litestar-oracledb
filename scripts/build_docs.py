...
def checkout(branch: str) -> Generator[None, None, None]:
    subprocess.run(["git", "checkout", branch], check=True)  # noqa: S607, S603
    yield
    subprocess.run(["git", "checkout", "-"], check=True)  # noqa: S607, S603


def build(output_dir: str) -> None:
    subprocess.run(["make", "docs"], check=True)  # noqa: S607, S603
